...
static int HAND_HEIGHT = 75;
static int DRAW_HEIGHT = 115;
static int DRAW_BORDER = 10;
...
+(CGPoint)BoardLocation {
    CGSize winSize = [[CCDirector sharedDirector] winSize];
    return ccp(winSize.width/2, winSize.height/2 + 65);
}
...
+(CGPoint)scoreLocation:(int)scoreIndex {
    CGSize winSize = [[CCDirector sharedDirector] winSize];
    int scoresPerRow = 4;
    int x = scoreIndex%scoresPerRow * (winSize.width/scoresPerRow) + 38;
    int y = winSize.height - ((scoreIndex/scoresPerRow + 1)* (15));
    return ccp(x, y);
}
+(CGPoint)acrossHand {
    CGSize winSize = [[CCDirector sharedDirector] winSize];
