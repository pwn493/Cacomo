...
-(NSArray *)renderSpritesAtLocation:(CGPoint)point {
    return [[self buildSprites:point.x :point.y] copy];
}
...
-(CCSprite *)renderStoneAtBoardLocation:(Stone *)stone :(CGPoint)point {
    ...
    stoneSprite.position = ccp(point.x+[self indexForX:uicol],point.y+[self indexForY:uirow]);
    
    return stoneSprite;
}

-(NSArray *)tempAddPotenteialMove:(Stone *)potStone :(CGPoint)point {
    NSMutableArray *sprites = [self buildSprites:point.x :point.y];
    potStone.potentialMove = true;
    CCSprite *potSprite = [potStone render];
    int uirow = self.size - potStone.row - 1;
    int uicol = potStone.col;
    potSprite.position = ccp(point.x+[self indexForX:uicol],point.y+[self indexForY:uirow]);
...
-(int)indexForX:(int)x {
    ...
    return (x*76 - 76*offset);
}
-(int) indexForY:(int)y {
    double offset = (self.size == 8) ? 3.5 : 2.5;
    return (y*76 - 76*offset);
}
@end
