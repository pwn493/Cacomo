...
-(void)onPotentialPlayReady:(NSNotification *)notification {
    ...
    NSArray *sprites = [board tempAddPotenteialMove:stone :[SpritePositions BoardLocation]];
...
-(void)renderBoard {
    Board *board = [GameState getBoard];
    NSArray *sprites = [board renderSpritesAtLocation :[SpritePositions BoardLocation]];
    for (id sprite in sprites) {
        [self addChild:(CCSprite*)sprite];
    }
}
...
-(void) renderPlayAction:(Stone *) stone {
    ...
    CCSprite *newMove = [[GameState getBoard] renderStoneAtBoardLocation:stone :[SpritePositions BoardLocation]];
    newMove.scale = 1.6;
