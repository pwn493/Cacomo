...
static int FONT_SIZE = 62;
@synthesize state = _state;
@synthesize nameMap = _nameMap;
static int HEIGHT_OFFSET = 30;
static int MENU_SIZE = 70;
...
-(void) renderStartButton {
    ...
    menu.position = ccp(winSize.width/2, start.contentSize.height / 2 + 80);
    [self addChild:menu];
}
-(void) renderPlayers {
    ...
    label.position = [self titlePosition:0 title:label];
    [self addChild: label];

    menu.position = [self optionsPosition:0 menuItems:[[NSArray alloc] initWithObjects:two,three,four, nil]];
...
-(void) renderDifficulty {
    ...
    label.position = [self titlePosition:1 title:label];
    [self addChild: label];

    menu.position = [self optionsPosition:1 menuItems:[[NSArray alloc] initWithObjects:easy,medium,hard, nil]];
    [menu alignItemsHorizontallyWithPadding: 30.0f];
    [self addChild:menu];
}
-(void) renderBoardSize {
    ...
    label.position = [self titlePosition:2 title:label];
    [self addChild: label];

    menu.position = [self optionsPosition:2 menuItems:[[NSArray alloc] initWithObjects:big,small, nil]];
    [menu alignItemsHorizontallyWithPadding: 30.0f];
    [self addChild:menu];
}
-(void) renderHandPassing {
    ...
    label.position = [self titlePosition:3 title:label];
    [self addChild: label];

    menu.position = [self optionsPosition:3 menuItems:[[NSArray alloc] initWithObjects:on,off, nil]];
    [menu alignItemsHorizontallyWithPadding: 30.0f];
    [self addChild:menu];
}
...
-(CGPoint) titlePosition:(int)index title:(CCLabelTTF *)label {
    CGSize winSize = [[CCDirector sharedDirector] winSize];
    int height = 60 + index * 180;
    return ccp(30 + label.texture.contentSizeInPixels.width/2, winSize.height - height);
}
-(CGPoint) optionsPosition:(int)index menuItems:(NSArray *)items {
    CGSize winSize = [[CCDirector sharedDirector] winSize];
    int height = 140 + index * 180;
    
    int size = [self getMenuWidth:30 items:items];
    return ccp(33 + size / 2, winSize.height - height);
}
@end
